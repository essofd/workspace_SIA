...
public class ReportLog extends TestUtils {
	...
	public static BaseColor baseCor = null;
	...
	public void apontarRelatorioHtml(String nomeCaso) throws IOException {
		...
		report = new ExtentReports(Utils.userDir + Utils.html + DataDriven.nomeCaso + "_" + Utils.gerarDataHoraAtual()
				+ "_" + Utils.obterNomeBrowser() + "_" + Utils.obterVersaoBrowser() + "_" + Utils.osName + ".html");
		...
		/********************************************************************************
		 * Carrega config.properties e lê o arquivo xml de configuração do Extent
		 * Reports
		 ********************************************************************************/
	...
	public void gerarRelatorioPDF(String resultado) throws Exception {
		...
		try {
			...
			/**
			 * Configura o path do relatório .pdf e da evidência de teste ,logo da Estacio e
			 * fonte utilizada no checkbox do relatório.
			 */
			String File = Utils.userDir + Utils.pdf + DataDriven.nomeCaso + "_" + Utils.gerarDataHoraAtual() + "_"
					+ Utils.obterNomeBrowser() + "_" + Utils.obterVersaoBrowser() + "_" + Utils.osName + ".pdf";
			String IMG = Utils.userDir + Utils.html + DataDriven.nomeCaso + "_" + Utils.gerarDataHoraAtual() + "_"
					+ Utils.obterNomeBrowser() + "_" + Utils.obterVersaoBrowser() + "_" + Utils.osName + ".jpg";

			String IMGCaixa = Utils.userDir + "\\Utilitarios\\logo-empresa\\imglogoestacio2.jpg";
			String fonte = Utils.userDir + "\\Utilitarios\\fonts\\wingding.ttf";
			...
			if (res == "passou") {
				baseCor = BaseColor.GREEN;
			} else if (res == "falhou") {
				baseCor = BaseColor.RED;
			}

			Font font = new Font(base, 16f, Font.BOLD, baseCor);
			...
			document.add(new Paragraph("Status do Teste: " + res));
	...
	public static void iniciarGravacaoVideoExecucaoCasoTeste() throws ATUTestRecorderException {
		recorder = new ATUTestRecorder(Utils.userDir + Utils.video,
				DataDriven.nomeCasoTeste + "_" + Utils.gerarDataHoraAtual() + "_" + Utils.obterNomeBrowser() + "_"
						+ Utils.obterVersaoBrowser() + "_" + Utils.osName,
				false);
		// System.out.println("inicio video");
		recorder.start();
	}

	public static void finalizarGravacaoVideoExecucaoCasoTeste() throws ATUTestRecorderException {
		//System.out.println("fim video");
		
		
		if (SuiteTeste.VIASUITE!="SIM") { //nao veio pela suite
			recorder.stop();
		} else {
			SuiteTeste.itestes++;
			if (SuiteTeste.itestes==N) {
				recorder.stop();
			}
		}
